...
class CFRAgent():
    ...
    def traverse_tree(self, probs, player_id):
        ...
        for action in legal_actions:
            ...
            self.average_policy[obs][action] += self.iteration * \
                player_prob * action_prob
        return state_utility
    ...
    def regret_matching(self, obs):
        ...
        if positive_regret_sum > 0:
            for action in range(self.env.action_num):
                action_probs[action] = max(
                    0.0, regret[action] / positive_regret_sum)
        else:
            for action in range(self.env.action_num):
    ...
    def eval_step(self, state):
        ...
        '''
        probs = self.action_probs(state['obs'].tostring(
        ), state['legal_actions'], self.average_policy)
    ...
    def save(self):
        ...
        average_policy_file = open(os.path.join(
            self.model_path, 'average_policy.pkl'), 'wb')
        ...
        iteration_file = open(os.path.join(
            self.model_path, 'iteration.pkl'), 'wb')
    ...
    def load(self):
        ...
        average_policy_file = open(os.path.join(
            self.model_path, 'average_policy.pkl'), 'rb')
        ...
        iteration_file = open(os.path.join(
            self.model_path, 'iteration.pkl'), 'rb')
