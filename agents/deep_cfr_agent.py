...
class DeepCFR():
    ...
    def __init__(self,
        ...
        for dim in list(policy_network_layers):
            fc = tf.contrib.layers.fully_connected(
                fc, dim, activation_fn=tf.tanh)
        action_logits = tf.contrib.layers.fully_connected(
            fc, self._num_actions, activation_fn=None)
        ...
        self._loss_policy = tf.reduce_mean(
            tf.losses.mean_squared_error(
                labels=self._action_probs_ph *
                tf.math.sqrt(self._iter_ph)[:, tf.newaxis],
        ...
        with tf.variable_scope(scope+'_advantage'):
            for _ in range(self._num_players):
                ...
                for dim in list(advantage_network_layers):
                    fc = tf.contrib.layers.fully_connected(
                        fc, dim, activation_fn=tf.tanh, reuse=False)
                    i += 1
                self._advantage_outputs.append(tf.contrib.layers.fully_connected(
                    fc, self._num_actions, activation_fn=None))
        ...
        for p in range(self._num_players):
            ...
            pred = self._advantage_outputs[p] * \
                tf.math.sqrt(self._iter_ph)[:, tf.newaxis]

            batch_size = tf.shape(self._info_state_ph)[0]
            gather_indices = tf.range(batch_size) * \
                tf.shape(pred)[1]+self._action_ph[p]
            action_predictions = tf.gather(
                tf.reshape(pred, [-1]), gather_indices)

            loss = tf.reduce_mean(tf.losses.mean_squared_error(
                labels=lbl, predictions=action_predictions))
            self._loss_advantages.append(loss)

            self._optimizer_advantages.append(
                tf.train.AdamOptimizer(learning_rate=learning_rate))
            self._learn_step_advantages.append(
                self._optimizer_advantages[p].minimize(self._loss_advantages[p]))
    ...
    def train(self):
        ...
        adv_loss = [self.advantage_losses[p][-1]
                    for p in self.advantage_losses.keys() if self.advantage_losses[p][-1] is not None]
    ...
    def reinitialize_advantage_networks():
        ...
        '''
        advantage_vars = [
            v for v in tf.global_variables() if 'advantage' in v.name]
    ...
    def _traverse_game_tree(self, state, player, count=0):
        ...
        if current_player == player:
            ...
            for action in actions:
                ...
                for a_ in actions:
                    sampled_regret[action] -= strategy[a_] * \
                        expected_payoff[a_][player]
            for act in actions:
                self._advantage_memories[player].add(AdvantageMemory(
                    state['obs'].flatten(), self._iteration, sampled_regret[act], act))
            players_payoff = [max(expected_payoff[act_])
                              for act_ in expected_payoff.keys()]
    ...
    def _sample_action_from_advantage(self, state, player):
        ...
        cumulative_regret = np.sum([advantages[action]
                                    for action in legal_actions])
        matched_regrets = np.array([0.] * self._num_actions)
        for action in legal_actions:
            if cumulative_regret > 0.:
                matched_regrets[action] = advantages[action] / \
                    cumulative_regret
            else:
